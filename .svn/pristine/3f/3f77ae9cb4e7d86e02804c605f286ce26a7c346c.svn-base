...
<script>
    $("#datalist1").datalist({
        checkbox: true,
        singleSelect: false,
        url: '${ctx}/proj/proj/getUnselectedUser/' + '${projectSetId}',
        columns: [[
    ...
    $("#datalist2").datalist({
        checkbox: true,
        singleSelect: false,
        url: '${ctx}/proj/proj/getSelectedUser/' + '${projectSetId}',
        columns: [[
            {field: 'id', title: 'id', hidden: true},
            {field: 'name', title: '姓名', width: 100}
        ]]
    ...
    function onSelOk() {
        ...
        $.post("${ctx}/proj/proj/addMembers", {projectSetId: projectSetId, id: ids}, function (data) {
            if (data.success) {
                closeWindow();
                $("#dg").datagrid('reload');
            } else {
