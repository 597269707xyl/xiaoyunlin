...
<script>
    ...
    $(function () {
        $('#projectTree').tree({
            animate: true,
            url: '${ctx}/func/exec/tree?mark=recv&caseType=k&id=-1',
            onlyLeafCheck: true,
            cascadeCheck:false,
            onSelect: function (node) {
                ...
                $(this).tree('options').url = '${ctx}/func/exec/tree?mark=recv&caseType=k&type='+type;
    ...
    function addItem(){
        var node = $('#projectTree').tree('getSelected');
        if (node) {
            commonShowWindow(null, '增加功能测试项', '${ctx}/casebase/addItemPage?type=k&mark=recv', 400, 220, true, false, false);
        } else {
    ...
    function editItem() {
        ...
        if (ns.length>0 || node) {
            ...
            commonShowWindow(param, '修改功能测试项', '${ctx}/casebase/addItemPage?type=k&mark=recv', 400, 220, true, false, false);
        } else {
