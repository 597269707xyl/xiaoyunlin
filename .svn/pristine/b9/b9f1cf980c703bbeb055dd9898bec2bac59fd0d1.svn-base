...
<script>
    function setMyData(param){
        ...
        $("#dg-reply-list").datagrid({
            ...
            title: '应答报文列表',
            fitColumns: true,
            url:'${ctx}/sim/message/getResponseMsgs?requestId='+param.id,
            height: 350,
            columns: [[
            ...
            toolbar: [{
                ...
                handler: function(){
                    $('#window-replyset').window({
                        ...
                        width: 480,
                        height: 420,
                        href: '${ctx}/sim/message/listReponseMsgs?type=XML',
                        modal: true,
                        minimizable: false,
                        maximizable: false,
                        shadow: false,
                ...
                handler: function(){
                    ...
                    $.post("${ctx}/sim/message/deleteResponses", {requestId: param.id, ids: ids}, function (data) {
                        if (data.success) {
                            $("#dg-reply-list").datagrid('reload');
                        } else {
