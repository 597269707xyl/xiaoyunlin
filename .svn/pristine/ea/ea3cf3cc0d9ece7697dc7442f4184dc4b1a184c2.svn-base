...
<div>
    <table>
        ...
        <tr>
            <td><a href="javascript:showexpecteddatafromdata()">添加预期值</a>
...
<script>
    ...
    function setData(param){
        ...
        if(rows.length > 0){
            document.getElementById('isShowRuleData').style.display = '';
        } else {
            document.getElementById('isShowRuleData').style.display = 'none';
        }
    }
...
//        params.replyDataList = replyDataList;
        ...
        if(typeof (usecaseId) == 'undefined' || usecaseId == '' || usecaseId == 0){
            ...
            $.messager.progress();
            params.usecaseDataList = encodeURI(params.usecaseDataList);
            $.ajax({
